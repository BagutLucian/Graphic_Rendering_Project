...
{
	myWindow.Create(1024, 768, "Oasis");
...
void initObjects() {
	...
	FancyTent.LoadModel("objects/FancyTent/FancyTent.obj");
...
//aici am creat functii separate pentru a render-ui fiecare obiect in parte pentru lizibilitate

void renderBariera(gps::Shader shader, bool depthPass)
{
	shader.useShaderProgram();
...
void renderMasina(gps::Shader shader, bool depthPass)
{
	shader.useShaderProgram();
...
void renderFancyTent(gps::Shader shader, bool depthPass)
{
	shader.useShaderProgram();
	model = glm::translate(model, glm::vec3(-21.17f, -1.19f, -12.2675f));
	glUniformMatrix4fv(glGetUniformLocation(shader.shaderProgram, "model"), 1, GL_FALSE, glm::value_ptr(model));
...
void renderScene() {
	...
	renderBariera(depthMapShader, 1);
	renderMasina(depthMapShader, 1);
	...
	if (showDepthMap) {
		...
		renderBariera(myCustomShader, false);
		renderMasina(myCustomShader, false);
